...
int EzATMcmd :: InitEzATMcmd( )
{
    ...
    if( openDebug( logpath, "ezatmcmd" ) < 0 )
            exit( 0 );

	if( InitEzChLocateUtil( this, "ezatmcmd" ) < 0 )
    {
    ...
    if( LocateChannel( CONTROLLERCHANNEL ) < 0 )
    {
            trace( "Error in locating EzATMController Channel" );
            trace( "^^^^^^^^^^^EzATMcmd Ended Abnormally^^^^^^^^^^^^^" );
            closeDebug();
            TaskExit();
   	}
	...
	if( dbutil.initDBUtil( "ezatmcmd", this ) < 0 )
	{
...
void EzATMcmd :: postControllerMsg( struct EzAtmController *atmctrlmsg )
{
	...
	if( WriteChannel( (char*)atmctrlmsg, sizeof( struct EzAtmController ) ) < 0 )
	{
...
void EzATMcmd :: postManagerMsg( struct EzAtmMgrQMsg *atmmgrmsg )
{
    ...
    if( LocateChannel( ATMMGRCHANNEL ) < 0 )
    {
    	trace( "Error in locating %s Channel", ATMMGRCHANNEL );
        trace( "^^^^^^^^^^^EzATMcmd Ended Abnormally^^^^^^^^^^^^^" );
        closeDebug();
        TaskExit();
   	}

    if( WriteChannel( (char *)atmmgrmsg, sizeof( struct EzAtmMgrQMsg ) ) < 0 )
	{
